...
/*   Updated: 2022/02/23 01:52:40 by ngenadie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	dynarray_realloc(t_dynarray *dynarray, size_t new_byte_size)
{
	...
	memcpy(ptr, dynarray->list, dynarray->cell_size * dynarray->nb_cells);
	free(dynarray->list);
	dynarray->list = ptr;
...
int	dynarray_extract(t_dynarray *darr, uint64_t index, uint64_t nb_ext)
{
	...
	else
	{
		...
		memcpy(darr->tmp, ptr, (darr->nb_cells - nb_ext - index) * cell_size);
		memcpy(darr->list + index * cell_size, darr->tmp, nb_ext * cell_size);
	...
	if (index != darr->nb_cells && index != 0)
	{
		...
		if ((darr->nb_cells + nb_ncells) * cl_s > darr->byte_size)
			if (dynarray_realloc(darr, (darr->nb_cells + nb_ncells) * cl_s))
				return (-1);
		memcpy(darr->tmp, darr->list + (index * cl_s), lf_cl_s);
		memcpy(darr->list + index * cl_s, ncells, nb_ncells * cl_s);
		memcpy(darr->list + (index + nb_ncells) * cl_s, darr->tmp, lf_cl_s);
