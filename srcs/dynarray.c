...
/*   Updated: 2022/02/23 01:52:23 by ngenadie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{	
	if (dynarray == NULL || cell_size == 0)
		return (-1);
	memset(dynarray, 0, sizeof(t_dynarray));
				...
				bool way)
{
	...
	if (way)
	{
		memcpy(darr->tmp, darr->list, nb_cells * darr->cell_size);
		memcpy(darr->list, new_cells, new_size);
		memcpy(darr->list + new_size, darr->tmp, nb_cells * darr->cell_size);
	}
	else
		memcpy(darr->list + nb_cells * darr->cell_size, new_cells, new_size);
